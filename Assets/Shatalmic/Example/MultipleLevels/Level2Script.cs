...
public class Level2Script : MonoBehaviour
{
    ...
    public TMP_Text OutputDataText;
    ...
    public void OnSubscribeClick(int buttonID)
    {
        if (buttonID >= 0 && buttonID < 4)
        {
            ...
            {
                BluetoothLEHardwareInterface.Log("subscribing to: " + subscribedService + ", " +
                                                 subscribedCharacteristic);

                OutputDataText.text += "subscribing to: " + subscribedService + ", " + subscribedCharacteristic + '\n';

                BluetoothLEHardwareInterface.SubscribeCharacteristic(device.Address, subscribedService,
                    subscribedCharacteristic, null,
                    (characteristic, bytes) =>
                    {
                        BluetoothLEHardwareInterface.Log("received data: " + characteristic);
                        
                        OutputDataText.text += "received data: " + characteristic + '\n';
                    });
            }
        }
    }
        ...
        if (buttonID >= 0 && buttonID < 4)
        {
            ...
            if (device != null && button != null)
            {
                if (button.text.Contains("connected"))
                {
                    if (!string.IsNullOrEmpty(subscribedService) && !string.IsNullOrEmpty(subscribedCharacteristic))
                    {
                        BluetoothLEHardwareInterface.UnSubscribeCharacteristic(device.Address, subscribedService,
                            subscribedCharacteristic, (characteristic) =>
                            {
                                ...
                                BluetoothLEHardwareInterface.DisconnectPeripheral(device.Address,
                                    (disconnectAddress) => { button.text = device.Name; });
                            });
                    }
                    else
                    {
                ...
                {
                    BluetoothLEHardwareInterface.ConnectToPeripheral(device.Address, (address) => { }, null,
                        (address, service, characteristic) =>
                        {
                            if (string.IsNullOrEmpty(Services[buttonID]) &&
                                string.IsNullOrEmpty(Characteristics[buttonID]))
                            {
